...
const AddActivity: FC<DialogProps & { onClose: () => void }> = ({
  onClose,
  open,
  ...props
}) => {
  ...
  const handleSubmit = async (data: IActivity) => {
    try {
      const docRef = doc(collections.activities.ref);
      await SetDocument<IActivity & { id: string }>({
        data: { ...data, id: docRef.id } as any,
        docRef,
      });
          ...
          {({ values }: any) => (
            <Form>
              <Paper sx={{ p: 2, mb: 3 }}>
                ...
                <Typography sx={{ mb: 3 }}>
                  Add the activity details below
                </Typography>
                <Box sx={{ mb: 3 }}>
                  <Field
                    component={TextField}
